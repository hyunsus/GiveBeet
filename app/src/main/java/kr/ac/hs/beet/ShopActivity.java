...
public class ShopActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       //하단 버튼을 없애는 기능
        ...
        txt_beet = findViewById(R.id.txt_Beet);

        // 사용자의 비트 수를 가져오기 (DB생성 후 에는 select로 찾아오기)
        Beet = Integer.parseInt(txt_beet.getText().toString());

        clothgridview = (GridView) findViewById(R.id.ClothGrid);
        clothAdapter = new ClothGridViewAdapter();

        objectgridview =(GridView) findViewById(R.id.ObjectGrid);
        objectAdapter = new ObjectGridViewAdapter();
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                   /* Toast.makeText(context, clothItem.getPrice()+" 번 - "+clothItem.getName()+" 입니당! ", Toast.LENGTH_SHORT).show();*/
                    ...
                    Intent intent;
                    if( Beet < price) {
                        intent = new Intent(getApplicationContext(), ShopPopupNOKActivity.class);
                    }
                    else{
                        intent = new Intent(getApplicationContext(), ShopPopupOKActivity.class);
